...
ptype = :runandtumble # particle label used to sorting and plotting
...
for (x, y) in square_lattice_with_φ(φ = φ, L_x = L_x, L_y = L_y, radius = R)
    push!(simulation.particles, Particle(ptype = ptype, x = x, y = y, R = R, D_trans = D_trans, active_force = RunAndTumble(γv = γv, α = α)))
end
pgroup_all = group_by_type(simulation.particles; ptype = :runandtumble)
...
plot_history!(simulation.history; frame_size = (600, 600), xlim = [0.0, simulation.L_x], ylim = [0.0, simulation.L_y], colors = Dict(:runandtumble => "black"), folder = "frames/Example_RunAndTumble/")
